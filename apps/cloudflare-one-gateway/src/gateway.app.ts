...
import type { Env } from './gateway.context'
...
export class ZeroTrustGatewayMCP extends McpAgent<Env, State, Props> {
...
export default {
	fetch: async (req: Request, env: Env, ctx: ExecutionContext) => {
		if (env.ENVIRONMENT === 'development' && env.DEV_DISABLE_OAUTH === 'true') {
			return await handleDevMode(ZeroTrustGatewayMCP, req, env, ctx)
		}

		return new OAuthProvider({
			apiHandlers: {
				'/mcp': ZeroTrustGatewayMCP.serve('/mcp'),
				'/sse': ZeroTrustGatewayMCP.serveSSE('/sse'),
			},
			// @ts-ignore
			defaultHandler: createAuthHandlers({ scopes: ZeroTrustGatewayScopes, metrics }),
